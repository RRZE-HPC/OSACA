...
void kernel(DTYPE* a, const int repeat, const int cur_elementsy, const int cur_elementsx)
#ifndef MAIN
{
    for(int r=0; r < repeat; r++) {
        ...
        a[1] = a[cur_elementsx*(cur_elementsy-2)+cur_elementsx]; // to prevent subsequent kernel executions to overlap
        dummy((void*)a);
    }
}
#else
...
int main(int argc, char *argv[]) {
    ...
    for(int t=0; t<tests; t++) {
        ...
        sprintf(cur_region_name, "gs-2d-5pt_%i_%i_%i", cur_repeats, cur_elementsy, cur_elementsx);
        ...
        for(int warmup = 1; warmup >= 0; --warmup) {
            ...
            kernel(a, repeat, cur_elementsy, cur_elementsx);
        }
