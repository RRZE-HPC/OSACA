...
class ISASemantics(object):
    ...
    def assign_src_dst(self, instruction_form):
        ...
        if self._isa == "aarch64":
            for operand in [op for op in op_dict["source"] if isinstance(op, MemoryOperand)]:
                if operand.post_indexed or operand.pre_indexed:
        ...
        if self._has_store(instruction_form):
            instruction_form.flags.append(INSTR_FLAGS.HAS_ST)
    ...
    def get_reg_changes(self, instruction_form, only_postindexed=False):
        ...
        if only_postindexed:
            ...
            for op in instruction_form.operands:
                if isinstance(op, MemoryOperand) and op.post_indexed:
                    base_name = op.base.prefix + (op.base.regid if op.base.regid else "")
                    return {
                        base_name: {
                            "name": op.base.prefix + (op.base.regid if op.base.regid else ""),
                            "value": op.indexed_val.value,
                        }
                    }
            return {}
        ...
        for o in instruction_form.operands:
            if isinstance(o, MemoryOperand) and o.pre_indexed:
                ...
                base_name = o.base.prefix + o.base.name
        ...
        if isa_data is not None and "operation" in isa_data:
            for i, o in enumerate(instruction_form.operands):
                ...
                if isinstance(o, RegisterOperand):
                    o_reg_name = o.prefix + o.regid if o.regid else ""
    ...
    def substitute_mem_address(self, operands):
        ...
        return [self._create_reg_wildcard() if isinstance(op, MemoryOperand) else op for op in operands]

    def _create_reg_wildcard(self):
        """Wildcard constructor"""
        return MemoryOperand("WILDCARD")
